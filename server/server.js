...
server.post("/search-blogs", (req, res) => {
    ...
    if ( tag ) {
        findQuery = { tags: tag, draft: false, blog_id: { $ne: eliminate_blog } }    // "$ne" means "Not equal"
    } else if (query) {
...
server.post("/update-profile-img", verifyJWT, (req, res) => {
    ...
    User.findOneAndUpdate({ _id: req.user }, { "personal_info.profile_img": url })
        .then( () => { return res.status(200).json({ profile_img: url }); })
        .catch((err) => { return res.status(500).json({ error: err.message }); })
})
...
server.post("/update-profile", verifyJWT, (req, res) => {
        ...
        for (let i = 0; i < socialLinksArr.length; i++) {
            if (social_links[socialLinksArr[i]].length) {
                ...
                if ( !hostname.includes(`${socialLinksArr[i]}.com`) && socialLinksArr[i] != "website" ) {
                    ...
                    return res.status(403).json({ error: `The link you provided for ${socialLinksArr[i]} is invalid. Please enter a valid URL.`, })
                }
            }
        }
    } catch {
        // return res.status(500).json({ error: "You must provide full social links with http(s) included" })
        return res.status(500).json({ error: "You must include the all social media links, ensuring they start with http(s)" })
    }
    ...
    User.findOneAndUpdate({ _id: req.user }, updateObj, { runValidators: true, })
        .then( () => { return res.status(200).json({ username }); } )
        .catch( (err) => {
            if (err.code == 11000) {
                // return res.status(500).json({ error: "Username is already taken" });
                return res.status(500).json({ error: "Sorry, this username is already taken. Please choose a different one." });
            }
            return res.status(500).json({ error: err.message });
        })
})
...
//             .then( () => { return res.status(200).json({ id: blog_id }) })
//             .catch( err => { return res.status(500).json({ error: err.message }) } )
        ...
        .then( blog => {
            if ( !islikedByUser ) {
                ...
                Notification.findOneAndDelete({ user: user_id, blog: _id, type: "like" })
                    .then( data => { return res.status(200).json({ liked_by_user: false }) })
                    .catch( err => { return res.status(500).json({ error: err.message }) })
            }
        })
...
server.post("/get-blog-comments", (req, res) => {
    ...
    Comment.find( { blog_id, isReply: false } )
        ...
        .limit( maxLimit )
        .sort( { 'commentedAt': -1 } )
        .then( comment => { return res.status(200).json(comment) } )
        .catch( err => { 
            console.log(err) 
            return res.status(500).json( { error: err.message } )
        } )
...
server.post("/get-replies", (req, res) => {
    ...
    Comment.findOne({ _id })
        ...
        .select("children")
        .then((doc) => { return res.status(200).json( { replies: doc.children } ) } )
        .catch((err) => { return res.status(500).json( { error: err.message } ) } );
})
